...
import { Api, getApi, apiGetApps } from "./api.js";
import { HostList } from "./component/host/list.js";
import { showErrorPopup } from "./component/error.js";
import { buildUrl } from "./config_.js";


// Lock UI immediately while automatic startup is happening.
document.body.classList.add("loading");
...
async function startApp() {
    ...
    while (true) {
        ...
        try {
            ...
            console.info(
                `[Startup] Starting automatic setup. Attempt ${attempt + 1}`
            );
             ...
             * Get API instance.
             *
             * We create a fresh API instance on every retry so that
             * a failed/authentication state from the previous attempt
             * doesn't affect the next attempt.
             */
            const api = await getApi();


            /*
             * Create a fresh HostList on every attempt.
             */
            const hostList = new HostList(api);

            await hostList.forceFetch();

            const hosts = hostList.getHosts();
            ...
            /*
             * We expect exactly one host for the automated
             * streaming flow.
             */
            if (hosts.length === 0) {

                throw new Error(
                    "No streaming host is available."
                );
            }
            ...
            if (hosts.length > 1) {
                ...
                throw new Error(
                    "Multiple streaming hosts were found. Unable to automatically select a host."
                );
            }
            ...
            /*
             * Fetch applications available on the host.
             */
             ...
             * Find Desktop.
             *
             * Prefer app_id 1, otherwise search by title.
             */
            const desktopApp = apps.find(app =>
                app.app_id === 1 ||
                app.title.toLowerCase().includes("desktop")
            ...
            if (!desktopApp) {
                ...
                throw new Error(
                    "Desktop application was not found on the streaming host."
                );
            }


            console.info(
                "[Startup] Desktop found:",
                desktopApp.title,
                desktopApp.app_id
            );
             ...
             * Everything is ready.
             *
             * Remove the loading screen and go directly
             * to the streaming page.
             */
            ...
            console.error(
                `[Startup] Attempt ${attempt + 1} failed:`,
                error
            );
            ...
            console.info(
                `[Startup] Retrying in ${delay / 1000}s...`
            );
